s3_get_files <- function(s3_uri, local_s3_folder = getwd()) {
    ...
    cli::cli_alert_info("{n_files} file{?s} totaling {prettyunits::pretty_bytes(files_size)} will be downloaded to {local_s3_folder} ")
    ui_confirm()

    f <- function() {
        cli::cli_alert_info("Now downloading {n_files} file{?s}, {prettyunits::pretty_bytes(file_size)} in total size")
        sb <- cli::cli_status("{symbol$arrow_right} Downloading {n_files} files.")
